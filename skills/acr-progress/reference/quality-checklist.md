...
| PR3 | PROGRESS.md 结构正确 | progress SKILL.md 结构硬约束 | 全览是批次索引（每行一个批次）+ 批次块 = 元信息行 + 一张 6 列表格（Mx.y/概要/状态/RED/GREEN/备注）+ 永不重排 |
...
| # | 检查 | 硬约束来源 | 怎么验 |
|---|---|---|---|
| C1 | TDD RED 真看到红 | cycle SKILL.md + coding.md | 本批次表本 Mx.y 的 RED 列有 FAILED 输出片段 |
| C2 | TDD GREEN 全过 | cycle SKILL.md | 本批次表本 Mx.y 的 GREEN 列有 passed 输出片段 |
| C3 | GREEN 列 = 命令 + 输出片段 + 轮次 | progress SKILL.md 关键约束 | `pytest xxx -v → 4 passed in 0.06s（本轮 2026-09-18）` |
