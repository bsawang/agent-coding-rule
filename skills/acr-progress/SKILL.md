...
**SSOT 分工：全览表状态列是唯一真相源；批次块表格的 RED/GREEN 行是唯一证据存放地。**
...
| **批次块 = 标题 + 元信息行 + 一张 2 列表格** | 左列放标签（Mx.y标题/RED/GREEN/备注），右列放内容。每个 Mx.y 是一组 4 行。内容竖向排，永远不撑宽 |
| **2 列表格结构** | 表头 `条目` / `详情`。每组：`**Mx.y 状态 概要**`（标题行）+ RED 行 + GREEN 行 + 备注行 |
| **Mx.y 标题行格式** | `**M1.1 ✅ 创建待办条目**` = 编号 + 状态 emoji + spec §4 子需求标题，一次性提取，cycle 不改 |
| **批次块永不重排** | 归档只改标题状态后缀（`⏳` → `✅`），新批次直接 append 到末尾 |
| **批次号暗示顺序** | 批次 N+1 一定在批次 N 之后 |
| **⏳ 只在两处** | 全览批次状态列 + 全览子需求列里的 Mx.y + 当前进行中批次表的 Mx.y 标题行。已归档批次不许有 ⏳ |
| **RED/GREEN 必须短** | 只贴命令 + 关键输出片段（`6 passed in 0.03s`），不许贴完整报错。F1 对账核心是"输出在本轮对话里" |
| **cycle 要么 ✅ 要么塌方** | 中间断 = 塌方，写备注行。不搞"内部步骤游标" |
| **依赖序只在元信息行说一遍** | 表格里 Mx.y 组自然按依赖序排列即可 |
