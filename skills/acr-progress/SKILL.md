...
| **批次块 = 标题 + 一张 3 列表格** | 编号 / 概要 / 详情。前两行覆盖/依赖序（编号列空），然后每个 Mx.y 组：编号列写 `Mx.y ✅`、概要列写 spec 子需求标题、详情列放 RED/GREEN/备注内容 |
| **3 列表格结构** | 表头 `编号` / `概要` / `详情`。元信息行（覆盖/依赖序）编号列空；Mx.y 标题行只有前两列有值；RED/GREEN/备注行只有后两列有值 |
| **Mx.y 标题行** | 编号列 = `M1.1 ✅`（编号 + 状态 emoji），概要列 = spec §4 子需求标题，详情列空。progress 创建时一次性提取，cycle 不改 |
...
| **依赖序在表格第二行** | `依赖序` 行写一遍，Mx.y 组自然按这个顺序往下排 |
