...
| PR3 | PROGRESS.md 结构正确 | progress SKILL.md 结构硬约束 | 全览是批次索引（每行一个批次）+ 批次块 = 元信息行 + N个 Mx.y 子块（标题带状态、RED/GREEN/回写/待定 紧凑列表）+ 永不重排 |
...
| # | 检查 | 硬约束来源 | 怎么验 |
|---|---|---|---|
| C1 | TDD RED 真看到红 | cycle SKILL.md + coding.md | 本子块 RED 行有 FAILED 输出片段 |
| C2 | TDD GREEN 全过 | cycle SKILL.md | 本子块 GREEN 行有 passed 输出片段 |
