...
| 批次 | 状态 | 覆盖 | 子需求 | 备注 |
|---|---|---|---|---|
| 1 | ✅ | prd §M1 / spec §M1.1-M1.3 | M1.1创建✅ M1.2查询✅ M1.3删除✅ | 已归档 |
| 2 | ⏳ | prd §M2 / spec §M2.1-M2.2 | M2.1分类⏳ M2.2筛选⬜ | 进行中 |
| 3 | ⬜ | prd §M3 / spec §M3.1 | — | — |
...
| Mx.y | 概要 | 状态 | 证据 | 回写 | 待定 |
|---|---|---|---|---|---|
| M1.1 | 创建待办条目 | ✅ | ❌RED: ImportError...(2026-09-17 本轮) / 6 passed | — | — |
| M1.2 | 查询待办列表 | ✅ | 5 passed（2026-09-17 本轮） | — | — |
| M1.3 | 删除待办条目 | ✅ | 3 passed（2026-09-17 本轮） | — | — |
...
| **批次块 = 标题 + 批次级元信息行 + 一个表格** | 批次级信息（覆盖、依赖序）放在标题下面一行，不重复到表格列里 |
| **表格按内容长度排列** | 短字段（Mx.y、概要、状态）在前，长字段（证据）在最后。空字段（回写、待定）在末尾 |
