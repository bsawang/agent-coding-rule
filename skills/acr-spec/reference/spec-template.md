...
## 6. 待定项
| # | 待定 | 谁来定 | 何时必须定 | 影响（哪些 Mx.y 的验收/实现会被它决定） |
|---|---|---|---|---|
| 1 | ... | ... | ... | |
...
**MX 集合对账（必须两步都过）：**
- [ ] spec 的 `### Mx` 模块列表 = prd 的 `### Mx` 列表（grep diff 集合相等）？
- [ ] spec 每个 `#### Mx.y` 的 Mx 都在 prd 定义过（spec 没偷偷加 prd 没覆盖的模块）？

**验收与设计质量：**
- [ ] 每个子需求的验收标准可判吗 —— 可执行命令或可观察现象？
- [ ] 验收条件里有没有引用未定待定项？（有 → 待定项关了才能进 cycle）
- [ ] 详细设计（完整档=所有；标准档=被改动的）的六个字段都齐吗？错误路径和正常路径一样详细吗？
