...
对每个 Mx.y（progress 里的依赖序）：
  ...
  ③ 影响分析 —— 列出本改动可能影响的外部功能 + 可选测试计划（不真跑）
...
之前 cycle 跑完一个 Mx.y 要跑**全量测试**——这是假设"改一个功能可能破坏全局"。但 spec 已经通过 §3.2 模块边界约定了什么不许跨，plan 已经通过事实预检查扫了代码现状，**大漂移在流程前期已经被拦下了**。

cycle 只需要：
1. 列出**本改动可能直接影响**的外部功能（模块边界内的下游调用）
2. 给出**可选测试命令**（让开发者/CI 决定跑不跑）
3. 贴进 PROGRESS.md 证据列

**不真跑**。如果影响分析漏了、CI 挂了 → 那是 progress 收尾 F2+F3 三方 diff 该抓的事。
