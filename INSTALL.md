...
| 档位 | 判定句 | 做法 |
|------|--------|------|
| 快道 | F 编号集合与验收判据都不变 | spec 正文可就地改 + 记一条 spec 调整日志，免设计评审，开发→测试→收尾 |
| 标准档 | F 编号集合或验收判据变，概要设计不变 | spec 修订 + 详细设计 + 走完整循环 |
| 完整档 | 概要/架构设计变 | spec → 概要 → 架构 → 详细 + 走完整循环 |

**自检两问**：① F 编号集合要改吗（新增/删除功能点）？或任一 F 的验收判据要改吗（判什么、怎么判变了）？要改 → 不是快道。② 模块划分要改吗？要改 → 完整档。

**判据在语义层不在文件层**：只改 spec 的**表述**（措辞、背景、边界解释），只要每条验收标准的**判据等价**，仍是快道。
...
**档位判定**（改动落在哪层文档决定档位，详见 `acr-cycle` 入口）：F 编号集合与验收判据都不变 = 快道（可就地改 spec 表述 + 记调整日志）；F 集合或验收判据变 = 标准档；概要或架构变 = 完整档。自检两问：① F 编号集合或某条验收判据要改吗？② 模块划分要改吗？
